            ...
            if (parseInt(settings.count) > 0) {
                ...
                for (var i in json.algorithm) {
                    if (json.algorithm[i].status == "disabled") {
                        html += '<li class="' + json.algorithm[i].status + ' page-item" num="' + json.algorithm[i].num + '" ><a href="javascript:;" class="page-link">' + json.algorithm[i].text + '</a></li>';
                    }
                    else if (json.algorithm[i].status == "active") {
                        html += '<li class="' + json.algorithm[i].status + ' page-item" num="' + json.algorithm[i].num + '" ><a href="javascript:;" class="page-link">' + json.algorithm[i].text + '</a></li>';

                    } else if (json.algorithm[i].num == "more") {
                        html += '<li class="disabled page-item" num="' + json.algorithm[i].num + '" ><a href="javascript:;" class="page-link">' + json.algorithm[i].text + '</a></li>';
                    } else {
                        html += '<li class="page-item" num="' + json.algorithm[i].num + '" ><a href="javascript:;" class="page-link">' + json.algorithm[i].text + '</a></li>';
                    }
                }
                $(that).html(html);
                if (settings.skipPart == true) {
                    $(that).find(".pagination").append('<span class="text-muted" style="margin:5px;margin-left:10px;display:inline-block;font-size:18px;font-size:' + settings.fontSize + 'px">共有' + pageCount + '页/' + settings.count + '条</span><div style="display:inline-block" name="changePage"><span class="text-muted" style="margin:5px;margin-left:0px;display:inline-block;font-size:' + settings.fontSize + 'px"> <input type="text" id="toPageNumID" style="width: 50px;margin-right: 5px;"/>页<button id="pageRealBTID">确定</button>  </div>');
                }
            } else {
